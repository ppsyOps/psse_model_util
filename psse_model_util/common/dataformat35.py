...
DTYPE_RAW_DATA = \
    {'AREA': {"I": IdInt, "ISW": int, "PDES": float, "PTOL": float, "ARNAME": Name},
     ...
     'FACTS DEVICE': {"NAME": Name, "I": BusId, "J": BusId, "MODE": int, "PDES": float, "QDES": float, "VSET": float,
                      ...
                      "RMPCT": float,
                      "OWNER": OwnerId, "SET1": float, "SET2": float, "VSREF": int, "FCREG": int, "NREG": int,
                      "'MNAME'": Name},
     ...
     'GENERATOR': {"I": BusId, "ID": IdStr, "PG": float, "QG": float, "QT": float, "QB": float, "VS": float,
                   "IREG": int, "NREG": str, "MBASE": float, "ZR": float, "ZX": float, "RT": float, "XT": float,
                   "GTAP": float, "STAT": Status, "RMPCT": float, "PT": float, "PB": float, "BASLOD": int,
                   "O1": OwnerId, "F1": OwnerFraction, "O2": OwnerId, "F2": OwnerFraction, "O3": OwnerId,
                   "F3": OwnerFraction, "O4": OwnerId, "F4": OwnerFraction, "WMOD": int, "WPF": float},
     'GNE': {},
     'HEADER': {"IC": int, "SBASE": float, "REV": int, "XFRRAT": int, "NXFRAT": int,
                "BASFRQ": float, "TITLE1": str, "TITLE2": str},
     ...
     'LOAD': {"I": BusId, "ID": IdStr, "STAT": Status, "AREA": AreaId, "ZONE": ZoneId, "PL": float, "QL": float,
              ...
              "DGENP": float, "DGENQ": float, "DGENM": int, "LOADTYPE": str},
     ...
     'SUBSTATION': {},
     'SWITCHED SHUNT': {"I": BusId, "ID": SwShID, "MODSW": int, "ADJM": int, "ST": int, "VSWHI": float, "VSWLO": float,
                        "SWREG": int, "NREG": int, "RMPCT": float, "RMIDNT": str, "BINIT": float, "S1": int, "N1": int,
                        "B1": float, "S2": int, "N2": int, "B2": float, "S3": int, "N3": int, "B3": float, "S4": int,
                        "N4": int, "B4": float, "S5": int, "N5": int, "B5": float, "S6": int, "N6": int, "B6": float,
                        "S7": int, "N7": int, "B7": float, "S8": int, "N8": int, "B8": float},
     ...
     'TRANSFORMER': [
         ...
         {"WINDV1": float, "NOMV1": Voltage, "ANG1": float, "RATE1-1": Rating, "RATE1-2": Rating,
          ...
          "COD1": int, "CONT1": int, "NOD1": int, "RMA1": float, "RMI1": float, "VMA1": float, "VMI1": float,
          "NTP1": int, "TAB1": int, "CR1": float, "CX1": float, "CNXA1": float},
         {"WINDV2": float, "NOMV2": Voltage, "ANG2": float, "RATE2-1": Rating, "RATE2-2": Rating,
          ...
          "COD2": int, "CONT2": int, "NOD2": int, "RMA2": float, "RMI2": float, "VMA2": float, "VMI2": float,
          "NTP2": int, "TAB2": int, "CR2": float, "CX2": float, "CNXA2": float},
         {"WINDV3": float, "NOMV3": Voltage, "ANG3": float, "RATE3-1": Rating, "RATE3-2": Rating,
          ...
          "COD3": int, "CONT3": int, "NOD3": int, "RMA3": float, "RMI3": float, "VMA3": float, "VMI3": float,
          "NTP3": int, "TAB3": int, "CR3": float, "CX3": float, "CNXA3": float}],
     ...
     'VSC DC LINE': [
         ...
         {"IBUS": int, "TYPE1": int, "MODE1": int, "DCSET1": float, "ACSET1": float, "ALOSS1": float,
          ...
          "MAXQ1": float, "MINQ1": float, "VSREG1": int, "NREG1": int, "RMPCT1": float},
         {"JBUS": BusId, "TYPE2": int, "MODE2": int, "DCSET2": float, "ACSET2": float, "ALOSS2": float,
          ...
          "MAXQ2": float, "MINQ2": float, "VSREG2": int, "NREG2": int, "RMPCT2": float}],
